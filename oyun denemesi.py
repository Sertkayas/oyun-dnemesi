...
Yeni düşman tipleri tanımlandı
...
class Enemy(Actor):
    ...
    Args:
        parent(obj): Enemy nesnesinin tanımlandığı nesne.
        image(obj): Sprite imajı

    Mathods:
        set_pos(int, int): Sprite'ın ekrandaki yeri
    '''

    def __init__(self, parent, image, x = 0, y = 0, z = None ):
        super().__init__(image)
        self.is_hit = False
        self.parent = parent
        self.x = x
    ...
    def update(self):
        ...
        # Bu kısımlar her düşman tipi için ayrı olacak.
        self.move_pattern()
        self.attack_pattern()


    def move_pattern(self):
        ''' Daha sonra tipe göre belirlenecek '''
        pass
...
class EnemySimple(Enemy):
    ...
    def __init__(self, parent):
        image = parent.sheet.get_image("enemyRed1.png")
        super().__init__(parent, image)
...
class EnemyDiver(Enemy):
    ...
    def __init__(self, parent):
        image = parent.sheet.get_image("enemyGreen3.png")
        super().__init__(parent, image)
    ...
    def move_pattern(self):
        ...
        if (self.timer > 5):
            if self.alpha > 0:
                ...
                self.alpha += 1.5
        # Vektörler
        dx = v * math.sin(math.radians(self.alpha))
        dy = v * math.cos(math.radians(self.alpha))
        # Koordinatlar
        self.rect.y += dy
        self.rect.x += dx
        # Sprite
        self.image = pg.transform.rotate(self.image_orig, self.alpha)
    ...
    def set_opt(self, alpha):
        self.alpha = alpha

class EnemyDropper(Enemy):

    def __init__(self, parent):
        image = parent.sheet.get_image("enemyRed5.png")
        super().__init__(parent, image)
    ...
    def move_pattern(self):
        ...
        if self.rect.x < 2*v:
            ...
            self.rect.x = 2*v \
                         + math.sin(math.radians(self.timer-90)) \
                         * ((SCREEN_SIZE[0]-4*v-self.rect.width)/2) \
                         + (SCREEN_SIZE[0]-self.rect.width)/2
            self.timer +=1

        self.rect.y = self.y_offset \
                        + math.cos(math.radians(270-self.timer)) * 60

    def attack_pattern(self):
        target = self.parent.player.rect.x - self.rect.x
        if abs(target) < 10:
            self.fire()
    ...
    def core_engine(self):
        ...
        if (self.game_time > trigger):
            ...
            for attribs in attribs_list:
                if attribs['Type'] == "Sinus":
                    enemy = EnemySinus(self)
                elif attribs['Type'] == "Simple":
                    enemy = EnemySimple(self)
                elif attribs['Type'] == "Dropper":
                    enemy = EnemyDropper(self)
                elif attribs['Type'] == "Diver":
                    enemy = EnemyDiver(self)
                else:
                    raise Exception('Unknown Enemy', attribs['Type'])
                ...
                # Z paremetresi opsiyonel
                if 'z' in attribs:
                    enemy.set_opt(int(attribs['z']))
